...
<div class="grid grid-cols-12 gap-6 mt-5">
    ...
    <h2 class="intro-y text-lg font-medium mr-5 text-center">Deductions Management</h2>
        <div class="intro-x text-center xl:text-left">
            <a href="javascript:;" data-toggle="modal" data-target="#add" class="custom__button w-full text-white text-center hover:bg-blue-400 bg-theme-1 xl:mr-3 flex" type="submit"><i data-feather="plus"></i>New</a>
        </div>
        ...
        <table class="table table-report sm:mt-2">
            ...
            @forelse($deductions as $data)
                <tr class="intro-x">
                    ...
                    <td class="w-40">
                        <div class="flex">
                            <p class="font-small">{{$data->amount}}</p>
                        </div>
                    </td>
                    <td class="w-40">
                        <div class="flex">
                            <a href="javascript:;" data-toggle="modal" data-target="#edit" data-id="{{$data->id}}" data-description="{{$data->description}}" data-amount="{{$data->amount}}" class="edit-dialog custom__button w-30 text-white hover:bg-blue-400 bg-theme-9 xl:mr-3 flex"><i data-feather="edit"></i></a>
    ...
    <div class="modal__content">
        ...
        @csrf
            <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200 dark:border-dark-5">
                <h2 class="font-medium text-base mr-auto">Add Deductions</h2> 
            </div>
            <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                ...
                <div class="col-span-12 sm:col-span-12"> <label>Amount</label> <input type="text" name="amount" class="input w-full border mt-2 flex-1" placeholder="Input Amount"> 
                    @error('amount')
                        <div class="text-theme-6 mt-2">{{$message}}</div>
                    @enderror
                </div>
...
<div class="modal" id="edit">
    <div class="modal__content">
        ...
        @method('PUT')
            <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200 dark:border-dark-5">
                <h2 class="font-medium text-base mr-auto">Edit Deductions</h2> 
            </div>
            <div class="p-5 grid grid-cols-12 gap-4 row-gap-3">
                ...
                <div class="col-span-12 sm:col-span-12"> <label>Amount</label> <input type="text" id="amount" name="amount" class="input w-full border mt-2 flex-1" placeholder="Input Amount"> 
                    @error('amount')
                        <div class="text-theme-6 mt-2">{{$message}}</div>
                    @enderror
                </div>
...
<!-- Delete Deductions -->
 <div class="modal" id="delete">
    <div class="modal__content">
        <form action="{{route('deduction.destroy')}}" method="post">
            @csrf
            @method('DELETE')
...
<script>
    $(document).on("click", ".edit-dialog", function () {
        ...
        $('.modal__content #amount').val(amount);
    });
